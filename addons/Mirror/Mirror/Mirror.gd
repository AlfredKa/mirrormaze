...
@export var ResolutionPerUnit = 300

## The NodePath to the main camera
@export var MainCamPath: NodePath = "playa/TwistPivot/PitchPivot/Camera3D"
...
@export var cullMask : Array[int] = [2]
...
func _process(delta):
	...
	var baseSize = Vector2(2, 2) # The original size you designed for
	ResolutionPerUnit *= baseSize.length() / size.length()
